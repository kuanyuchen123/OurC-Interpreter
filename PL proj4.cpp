...
} // Ident::Prepare_Funct_Tokens_and_Idents()
...
void Ident::Redefine_Var( string type, string name, string array_size ) {
  Content content ;
  m_type = type ;
  m_name = name ;
  m_array = array_size ;
...
Content Get_Ident_Content( string name, string array_index ) {
  Content content ;
  bool content_not_found = true ;

  // Local Ident Table
  for ( int i = 0 ; i < g_IdentTable[g_IdentTable.size()-1].size() && content_not_found ; i++ ) {
    if ( g_IdentTable[g_IdentTable.size()-1][i].m_name == name &&
         g_IdentTable[g_IdentTable.size()-1][i].m_funct == "" ) {
      content_not_found = false ;
      if ( array_index == "-1" )
        content = g_IdentTable[g_IdentTable.size()-1][i].m_content ;
      else
        content = g_IdentTable[g_IdentTable.size()-1
                              ][i].m_content.m_content_array[atoi( array_index.c_str() )] ;
    } // if
  } // for

  // Global Ident Table
  for ( int i = 0 ; i < g_IdentTable[0].size() && content_not_found ; i++ ) {
    if ( g_IdentTable[0][i].m_name == name && g_IdentTable[0][i].m_funct == "" ) {
      content_not_found = false ;
      if ( array_index == "-1" )
        content = g_IdentTable[0][i].m_content ;
      else
        content = g_IdentTable[0][i].m_content.m_content_array[atoi( array_index.c_str() )] ;
    } // if
  } // for
...
void Update_Ident( Parameter parameter, Content content ) {
  bool not_updated = true ;
  // Local Ident Table
  for ( int i = 0 ; i < g_IdentTable[g_IdentTable.size()-2].size() && not_updated ; i++ ) {
    if ( g_IdentTable[g_IdentTable.size()-2
                     ][i].m_name == parameter.m_name &&
         g_IdentTable[g_IdentTable.size()-2][i].m_funct == "" ) {
      not_updated = false ;
      if ( parameter.m_array_index == "-1" )
        g_IdentTable[g_IdentTable.size()-2][i].m_content = content ;
      else
        g_IdentTable[g_IdentTable.size()-2
                    ][i].m_content.m_content_array[atoi( parameter.m_array_index.c_str() )] = content ;
    } // if
  } // for

  // Global Ident Table
  for ( int i = 0 ; i < g_IdentTable[0].size() && not_updated ; i++ ) {
    if ( g_IdentTable[0][i].m_name == parameter.m_name && g_IdentTable[0][i].m_funct == "" ) {
      not_updated = false ;
      if ( parameter.m_array_index == "-1" )
        g_IdentTable[0][i].m_content = content ;
      else
        g_IdentTable[0][i].m_content.m_content_array[atoi( parameter.m_array_index.c_str() )] = content ;
    } // if
  } // for
} // Update_Ident()
...
void Execute_Unsigned_unary_exp( Content &content ) {
  ...
  if ( token.m_type == "Identifier" ) {
    ...
    token = g_TokenList[0] ;
    if ( token.m_type == "(" ) {
      vector<Token>funct_tokens ;
      vector<Ident>funct_idents ;
      vector<Parameter>funct_referenced_parameters ;
      g_TokenList.erase( g_TokenList.begin() ) ;
      ...
      for ( int i = 0 ; i < g_IdentTable[0].size() ; i++ ) {
        if ( g_IdentTable[0][i].m_name == ident.m_value && g_IdentTable[0][i].m_funct != "" ) {
          ...
          funct_tokens = g_IdentTable[0][i].m_funct_tokens ;
        } // if
      } // for
      ...
      for ( int i = 0 ; i < funct_idents.size() ; i++ ) {
        string name = "" ;
        string array_index = "-1" ;
        if ( funct_idents[i].m_reference == true ) {
          name = g_TokenList[0].m_value ;
          g_TokenList.erase( g_TokenList.begin() ) ;
          token = g_TokenList[0] ;
          if ( token.m_type == "[" ) {
            g_TokenList.erase( g_TokenList.begin() ) ;
            array_index = g_TokenList[0].m_value ;
            g_TokenList.erase( g_TokenList.begin() ) ;
            g_TokenList.erase( g_TokenList.begin() ) ;
          } // if

          funct_idents[i].m_content = Get_Ident_Content( name, array_index ) ;
          Parameter parameter ;
          parameter.m_name = name, parameter.m_array_index = array_index ;
          funct_referenced_parameters.push_back( parameter ) ;
        } // if
        else {
          funct_idents[i].m_content = Execute_Actual_parameter_list() ;
          Parameter parameter ;
          funct_referenced_parameters.push_back( parameter ) ;
        } // else
      ...
      g_TokenList.insert( g_TokenList.begin(), funct_tokens.begin(), funct_tokens.end() ) ;
      content = Execute_Compound_statement() ;
      for ( int i = 0 ; i < funct_idents.size() ; i++ ) {
        if ( funct_idents[i].m_reference == true ) {
          Content updated_content ;
          updated_content = Get_Ident_Content( funct_idents[i].m_name, "-1" ) ;
          Update_Ident( funct_referenced_parameters[i], updated_content ) ;
        } // if
      } // for
